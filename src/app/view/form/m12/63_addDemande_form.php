	...
	$(function(){
        ...
        $('#reservation input[name=adding_reservation]').bind('click',function(){
			...
			$.each(data_salle_lists,function(){
				select_salle_html+='<option value="'+this['id']+'">'+this['nom']+'['+this['nombre_place']+']'+' - '+(this['description']).substr(0,40)+'</option>';
			});			
            ...
            var input_date_debut_html = '<input type="text" class="dateField" name="date_debut_'+index+'" placeHolder="jj/mm/yyyy"/><br/>';
            ...
            var select_sec_debut_html = '<select style="display:none;" name="sec_debut_'+index+'">'+sec_option+'</select>';

            var input_date_fin_html = '<input type="text" name="date_fin_'+index+'" readOnly="readOnly" placeHolder="jj/mm/yyyy"/><br/>';
            ...
            var select_sec_fin_html = '<select style="display:none;" name="sec_fin_'+index+'">'+sec_option+'</select>';
        	var html = '<tr>';
        		html+='<td width="5%"><input type="checkbox" name="reservation_'+index+'" checked="checked"></td>';
        		html+='<td width="5%">'+(index+1)+'</td>';
    ...
    <form action="" method="post" enctype="multipart/form-data">
		<fieldset >
			...
			<table class="form_table" style="text-align:left;">
				<tr>
					<td><span class="required">*</span>Structure</td>
					<td>
						<select name="structure" class="select2">
						<?php
						  $structures = Structure::all();
						  echo '<option value="0">--select--</option>';
						  foreach($structures as $structure)
						  {
						    if( (isset($_POST['structure'])&&$_POST['structure']==$structure->id()) || ($id_demande>0&&$demande->structure()->id()==$structure->id()) )
								echo '<option title="'.$structure->description().'" value="'.$structure->id().'" selected="selected">'.$structure->nom().'</option>';
							else
								echo '<option title="'.$structure->description().'" value="'.$structure->id().'">'.$structure->nom().'</option>';
						  }
						?>
						</select>
					</td>
				...
				<?php
				    ...
				    for($i=0;$i<$index;$i++)
				    {
				    	echo '<tr>';
				    		...
				    		echo '<td  width="40%">';
				    			...
				    			foreach ($salles as $key => $salle) {
				    				if(isset($_POST['salle_'.$i]) && $_POST['salle_'.$i]==$salle['id'])
				    					echo '<option value="'.$salle['id'].'" selected="selected">'.$salle['nom'].'['.$salle['nombre_place'].'] - '.substr($salle['description'], 0,40).'</option>';
				    				else
				    					echo '<option value="'.$salle['id'].'">'.$salle['nom'].'['.$salle['nombre_place'].'] - '.substr($salle['description'], 0,40).'</option>';
				    			}
				    			echo '</select>';
				    		echo '</td>';
				    		echo '<td width="25%">';
				    			echo '<input type="text" class="dateFiled" name="date_debut_'.$i.'" value="'.(isset($_POST['date_debut_'.$i])?$_POST['date_debut_'.$i]:'').'" placeHolder="jj/mm/yyyy"/><br/>';
				    			...
				    			echo '</select>';
				    			echo '<select style="display:none;" name="sec_debut_'.$i.'">';
				    				for($t=0;$t<60;$t++)
				    				{
				    		...
				    		echo '</td>';
				    		echo '<td  width="25%">';
				    			echo '<input type="text" name="date_fin_'.$i.'" value="'.(isset($_POST['date_fin_'.$i])?$_POST['date_fin_'.$i]:'').'" readOnly="readOnly" placeHolder="jj/mm/yyyy"/><br/>';
				    			...
				    			echo '</select>';
				    			echo '<select style="display:none;" name="sec_fin_'.$i.'">';
				    				for($t=0;$t<60;$t++)
				    				{
				    ...
				    if($index == 0 && $id_demande > 0)
				    {
				    	...
				    	foreach ($reservations as $key => $reservation) 
				    	{
				    		echo '<tr>';
					    		...
					    		echo '</td>';
					    		echo '<td width="25%">';
					    			echo '<input type="text" class="dateField" name="date_debut_'.$i.'" value="'.(Systeme::dateToFrench($reservation->dateDebutWithoutTimes())).'" placeHolder="jj/mm/yyyy"/><br/>';
					    			echo '<select name="heure_debut_'.$i.'">';
					    				for($t=0;$t<24;$t++)
					    				{
					    		...
					    		echo '</td>';
					    		echo '<td  width="25%">';
					    			echo '<input type="text" name="date_fin_'.$i.'" value="'.(Systeme::dateToFrench($reservation->dateFinWithoutTimes())).'" readOnly="readOnly" placeHolder="jj/mm/yyyy"/><br/>';
					    			echo '<select name="heure_fin_'.$i.'">';
					    				for($t=0;$t<24;$t++)
					    				{
