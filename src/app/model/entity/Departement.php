...
class Departement {
	...
	public static $db_table_name = 'app_departement';
	...
	public function dbSave($id_intervenant)
	{
		if($this->nom()!='')
		{
			if($this->id()>0) // update
			{
				return $this->db->update(self::$db_table_name,array('nom'=>$this->nom(),'description'=>$this->description()),array('id'=>$this->id()));
			}
			else if($id_intervenant>0)// nouvel enregistrement
			{
				$ok = $this->db->insertion(self::$db_table_name,'',$this->nom(),$this->description(), $id_intervenant,Systeme::now());
	...
	public static function all($pageSize=-1, $page=1)
	{
		...
		$rep = DepartementObjectBuilder::build(Systeme::array_key_values($records,'id'));
		return $rep;
	}
}
