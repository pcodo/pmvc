...
class Direction {
	...
	{
		$this->data = $this->db->queryOneRecord('select d.* from direction d where d.id='.$id);
	}
	...
	public static function  allAsRecords($pageSize=-1, $page=1)
	{
		self::init();
		$records = array();
		$query_string = ' select d.id from direction d order by d.designation ASC';
