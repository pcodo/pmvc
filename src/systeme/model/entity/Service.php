...
class Service {
	...
	{
		$this->data = $this->db->queryOneRecord('select s.* from service s where s.id='.$id);
	}
	...
	public function setDirectionId($id_direction)
	{
		if(is_numeric($id_direction) and $id_direction>0)
			$this->data['id_direction'] =  $id_direction;
		return $this;
	}
	public function direction()
	{
		return isset($this->data['id_direction'])?(new Direction($this->data['id_direction'])):new Direction(0);
	}
	...
	public static function  allAsRecords($pageSize=-1, $page=1)
	{
		self::init();
		$records = array();
		$query_string = ' select s.id from service s order by s.designation ASC';
