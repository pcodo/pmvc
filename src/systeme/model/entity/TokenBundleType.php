...
class TokenBundleType {
	...
	public function dbSave($id_intervenant)
	{
		...
		if($this->id()>0) // update
		{
			return $this->db->update(self::$db_table_name,array('name'=>$this->name(),'description'=>$this->description()),array('id'=>$this->id()));
		}
		else if($id_intervenant>0)// nouvel enregistrement
		{
			$ok = $this->db->insertion(self::$db_table_name,'',$this->name(),$this->description());
	...
	public static function all($pageSize=-1, $page=1)
	{
		...
		$rep = TokenBundleTypeObjectBuilder::build(Systeme::array_key_values($records,'id'));
		return $rep;
	}
