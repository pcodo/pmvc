...
$id_type_sm_query = $db->queryOneRecord('select id_type_sm from sous_menu where id_type_sm >0 and id_menu='.$id_menu.' LIMIT 1');
$id_type_sm = ($id_type_sm_query)?$id_type_sm_query['id_type_sm']:0;
?>
<script>
$(function(){
    $('input[name=adding]').click(function(){
		...
		options = [{value:0,text:'Ne pas afficher'},{value:1,text:'Afficher'}];
		...
		<fieldset>
			...
			<table class="form_table">
				...
				<tr>
					...
					<td>
						<select name="type_sm" <?php if($id_type_sm>0) echo 'disabled="disabled"'?> >	
							<?php
								$types_sm = $db->queryAllRecords('select * from type_sm');
								foreach($types_sm as $type_sm)
								{
									if( (isset($_POST['type_sm']) && $_POST['type_sm'] == $type_sm['id']) || ($id_type_sm>0 && $id_type_sm==$type_sm['id']) ) 
				...
				<thead>
					...
					<?php
						...
						while($sm = $reponse->fetch())
						{
						  ...
						  echo '<td><img src="'.$sm['icone_url'].'" width="20px" height="20px"/></td>';
						  ...
						  echo '<td>';
							echo '<select name="ex_show_at_startup_'.$i.'">';
								  echo '<option value="1" '.( ($sm['show_at_startup']==1)?'selected="selected"':'' ).'>Afficher</option>';
								  echo '<option value="0" '.( ($sm['show_at_startup']==0)?'selected="selected"':'' ).'>Ne pas afficher</option>';
							echo '</select>';
						  echo '</td>';
						  echo '<td><input type="file" name="ex_icone_'.$i.'" size=""/></td>';
						  echo '<td><input type="hidden" name="ex_icone_url_'.$i.'" id="ex_icone_url_'.$i.'" value="'.$sm['icone_url'].'"/></td>';
...
<?php
   if(isset($_POST['valider'])&&isset($_POST['menu'])&&$_POST['menu']!=''&&isset($_POST['categorie'])&&$_POST['categorie']>=0)
   {
	  ...
	  if(isset($_POST['id_menu'])) // si il existe la variable id_menu:: c'est qu'il y des sous_menu à modifier
	  {
			...
			$type_sm = ($id_type_sm>0)?$id_type_sm:$db->escape($_POST['type_sm']);
			...
			while(isset($_POST['ex_sm_'.$i]))
			{
			  ...
			  $show_at_startup = $db->escape($_POST['ex_show_at_startup_'.$i]);
			  $icone_url = $db->escape($_POST['ex_icone_url_'.$i]);
			  $file_url = 'uploads/sm/'.$id_sm; 
			  $upload_state = Systeme::upload_file('ex_icone_'.$i, $file_url, $extensions_autorisees, '100000');
			  if ($upload_state['etat'] == 'ok')
			  {
					$icone_url = $file_url . '.'.$upload_state['extension'];
			  }
			  $db->update('sous_menu', array('designation'=>$sm,'description'=>$desc,'position'=>$position,'active_state'=>$sm_active_state,'show_help'=>$sm_show_help,'on_passwd_access'=>$on_passwd_access,'show_at_startup'=>$show_at_startup,'icone_url'=>$icone_url,'id_type_sm'=>$type_sm),array('id'=>$id_sm));
			  $i++;
			}
			...
			while(isset($_POST['sm_'.$i])&&$_POST['sm_'.$i]!='')
			{
			    ...
			    $icone_url = '';
				if($db->insertion('sous_menu','',$id_menu_val,$sm,$desc,$lastId+(++$i),$sm_active_state,$sm_show_help,$on_passwd_access,$show_at_startup,$icone_url,$type_sm))
				{
