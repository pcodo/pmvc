...
?>
<div align="center" style="border: inset thin;border-top: outset thin;border-left: outset thin;min-height:300px;">
    ...
    <table border="0" id="dataTable" class="display datatable">
        ...
        while ($menu = $menus->fetch()) {
			...
			echo '</td>';
			echo '<td>';
			echo '<a href="index.php?m='.$_GET['m'].'&sm=5_update_advanced&id_menu='.$menu['id'].'">Attributs</a>';
			echo '</td>';
			echo '</tr>';
        }
...
if(isset($_POST['valider'])&&isset($_POST['menu_id_max']))
{
	...
	if($menu_id_max!='')
	{
		...
		{	
			...
			{
				$ok = $db->update('menu', array('id_categorie'=>$_POST['cat_menu_'.$i]),array('id'=>$i));
				if($ok)
				{
