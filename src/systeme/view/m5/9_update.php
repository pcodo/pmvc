...
/*
  ...
  Auteur: CODO Paterne
*/
if(!isset($_GET['id_poste'])) exit();
$id_poste = $db->escape($_GET['id_poste']);
$poste = $db->queryOneRecord('select * from poste where id='.$id_poste);
?>
<script>
$(function(){
...
<div align = "center" class="radius">
	<form action="" method="post">
		<fieldset >
			<legend> Information du poste </legend>
			<table class="form_table">
				<tr>
					<td><label for="designation">DESIGNATION DU POSTE</label></td>
					<td><input type="text" name="designation" id="designation" size="30" value="<?php if(isset($_POST['designation'])) echo $_POST['designation']; else if(isset($poste['designation'])) echo $poste['designation'];?>"/></td>
				</tr>
				<tr>
					<td><label for="description">DESCRIPTION</label></td>
					<td><textarea name="description" id="description" cols="25"><?php if(isset($_POST['description'])) echo $_POST['description'];else if(isset($poste['description'])) echo $poste['description'];?></textarea></td>
				</tr>
				<tr>
					<td><label for="direction">DIRECTION</label></td>
					<td>
						<select name="direction" id="direction">
						<option value="-1">Veuillez selectionner la direction</option>
						<?php
						  $directions = $db->query('select * from direction');
						  $post_dir_name = '';
						  while($direct = $directions->fetch())
						  {
						    if((isset($_POST['direction'])&&$_POST['direction']==$direct['id'])||(isset($poste['id_direction'])&&$poste['id_direction']==$direct['id']))
						    {
				...
				<tr>
					...
					<td>
						...
						<option value="-1">Veuillez selectionner le service</option>
						<?php
						  $services = $db->query('select * from service');
						  $post_service_name = '';
						  while($service = $services->fetch())
						  {
						    if((isset($_POST['service'])&&$_POST['service']==$service['id'])||(isset($poste['id_service'])&&$poste['id_service']==$service['id']))
						    {
				...
				<tr>
					...
					<td>
						...
						<?php
						  ...
						  {
						    if( (isset($_POST['division'])&&$_POST['division']==$division['id']) || (isset($poste['id_division'])&&$poste['id_division']==$division['id']))
						    {
				...
				<tr>
					...
					<td>Oui<input type="radio" name="chef_direction" value="1" <?php if((isset($poste['chef_direction_state'])&&$poste['chef_direction_state']==1)||(isset($_POST['chef_direction'])&&$_POST['chef_direction']==1) ) echo 'checked="checked"'?> /> Non<input type="radio" name="chef_direction" value="0" <?php if((isset($poste['chef_direction_state'])&&$poste['chef_direction_state']==0) || (isset($_POST['chef_direction'])&&$_POST['chef_direction']==0) ) echo 'checked="checked"'?> /></td>
				</tr>
			</table>
		</fieldset>
		<fieldset>
...
<?php
   if(isset($_POST['valider'])&&isset($_POST['designation'])&&$_POST['designation']!=''&&isset($_POST['direction'])&&$_POST['direction']>=0)
   {
		...
		$db->update('poste',array('designation'=>$designation,'description'=>$description,'id_direction'=>$direction,'id_service'=>$service,'id_division'=>$division,'chef_direction_state'=>$chef_direction),array('id'=>$id_poste));
		...
		//redirection
		if(isset($_GET['action_source']) && $_GET['action_source'] =='poste_plus' )
			Systeme::redirect('index.php?m='.$_GET['m'].'&sm=62');
		else
