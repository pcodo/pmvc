...
<div align="center">
	...
	<table border="0" id="dataTable" class="display datatable" style="text-align:left">
        ...
        while ($poste = $postes->fetch()) {
			...
			echo '<td>';
			echo '<a href="index.php?m='.$_GET['m'].'&sm=9_update&action_source=poste_plus&id_poste='.$poste['id'].'">Modifier</a>';
			echo '</td>';
			echo '<td>';
			echo '<a href="index.php?m='.$_GET['m'].'&sm=62_configurer_poste&id_poste='.$poste['id'].'">Configurer</a>';
			echo '</td>';
			echo '<td title="Inactif">';
